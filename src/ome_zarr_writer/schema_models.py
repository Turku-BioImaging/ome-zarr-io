    ...
    def __post_init__(self):
        ...
        if self.type is not None:
            ...
            if self.type not in valid_types:
                raise ValueError(
                    f"Invalid axis type '{self.type}'. Must be one of: {', '.join(valid_types)}"
                )
...
class TranslationTransformation(CoordinateTransformation):
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary representation."""
        return {"type": self.type, "translation": self.translation}
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "coordinateTransformations": [
                t.to_dict() for t in self.coordinateTransformations
            ],
        }
...
def validate_tczyx_axis_ordering(axes: List[Axis]) -> None:
    ...
    for name in axis_names:
        if name not in valid_names:
            raise ValueError(
                f"Invalid axis name '{name}'. Valid names are: {', '.join(expected_order)}"
            )
    ...
    for axis in axes:
        ...
        if axis.type != expected_type:
            raise ValueError(
                f"Axis '{axis.name}' should have type '{expected_type}', got '{axis.type}'"
            )
...
class Multiscale:
    ...
    def __post_init__(self):
        ...
        if len(self.axes) < 2 or len(self.axes) > 5:
            raise ValueError(
                "Axes must have between 2 and 5 items (YX minimum, TCZYX maximum)"
            )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.coordinateTransformations is not None:
            result["coordinateTransformations"] = [
                t.to_dict() for t in self.coordinateTransformations
            ]
...
class OMEZarrImageMetadata:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "OMEZarrImageMetadata":
                ...
                for ct_data in ds_data["coordinateTransformations"]:
                    if ct_data["type"] == "scale":
                        coord_transforms.append(
                            ScaleTransformation(scale=ct_data["scale"])
                        )
                    elif ct_data["type"] == "translation":
                        coord_transforms.append(
                            TranslationTransformation(
                                translation=ct_data["translation"]
                            )
                        )

                datasets.append(
                    Dataset(
                        path=ds_data["path"], coordinateTransformations=coord_transforms
                    )
                )
        ...
        ome_metadata = OMEMetadata(
            multiscales=multiscales, version=ome_data["version"], omero=omero
        )
...
def create_axes(
    ...
    valid_combinations = {"yx", "zyx", "cyx", "czyx", "tyx", "tzyx", "tcyx", "tczyx"}
...
def create_yx_axes(
    pixel_size_x: float, pixel_size_y: float, unit: str = "micrometer"
) -> List[Axis]:
