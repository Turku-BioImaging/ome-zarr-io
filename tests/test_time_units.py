...
class TestTimeUnits:
    ...
    def test_create_axes_tczyx_with_time_unit(self):
        ...
        axes = create_axes("tczyx", 0.1, 0.1, 0.3, unit="nanometer", time_unit="millisecond")
        
        assert len(axes) == 5
        assert [ax.name for ax in axes] == ["t", "c", "z", "y", "x"]
        assert [ax.type for ax in axes] == ["time", "channel", "space", "space", "space"]
    ...
    def test_axis_validation_preserves_space_units(self):
        ...
        with pytest.raises(ValueError, match="Invalid unit .* for space axis"):
            create_axes("tzyx", 0.1, 0.1, 0.3, unit="invalid_space_unit", time_unit="second")
        
        # Invalid time unit should be caught
        with pytest.raises(ValueError, match="Invalid unit .* for time axis"):
            create_axes("tzyx", 0.1, 0.1, 0.3, unit="micrometer", time_unit="invalid_time_unit")
    ...
    def test_integration_with_existing_validation(self):
        ...
        axes = create_axes("tczyx", 0.1, 0.1, 0.3, unit="micrometer", time_unit="millisecond")
        ...
        assert t_axis.name == "t" and t_axis.type == "time" and t_axis.unit == "millisecond"
        assert c_axis.name == "c" and c_axis.type == "channel" and c_axis.unit is None
        assert z_axis.name == "z" and z_axis.type == "space" and z_axis.unit == "micrometer"
        assert y_axis.name == "y" and y_axis.type == "space" and y_axis.unit == "micrometer"
        assert x_axis.name == "x" and x_axis.type == "space" and x_axis.unit == "micrometer"
