...
def example_2_multichannel_metadata():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for i, info in enumerate(channel_info):
            ...
            channel = Channel(
                window=window, label=info["name"], color=info["color"], active=True
            )
        ...
        for info in channel_info:
            print(
                f"         {info['name']}: {info['description']} (color: #{info['color']})"
            )
        ...
        if (
            "ome" in attrs
            and isinstance(attrs["ome"], dict)
            and "omero" in attrs["ome"]
        ):
            ...
            if isinstance(omero_attrs, dict) and "channels" in omero_attrs:
                ...
                if isinstance(channels, list):
                    print(
                        f"      📊 Verified: {len(channels)} channels in OMERO metadata"
                    )
                else:
...
def example_4_multichannel_configurations():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for config in multichannel_configs:
            ...
            if "c" in config["dims"]:
                ...
                for i, channel_name in enumerate(config["channels"]):
                    ...
                    color = default_colors.get(
                        channel_name,
                        f"{(i*60)%256:02X}{(i*120)%256:02X}{(i*180)%256:02X}",
                    )

                    # Create channel with metadata
                    channel = Channel(
                        window=window, label=channel_name, color=color, active=True
                    )
                    omero_channels.append(channel)
            ...
            print(
                f"      Channels ({len(config['channels'])}): {', '.join(config['channels'])}"
            )
            print(f"      Dimensions: {config['dims']}")
            if omero_metadata:
                print(
                    f"      ✅ OMERO metadata written with {len(config['channels'])} channel definitions"
                )
            else:
