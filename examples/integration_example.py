...
def create_metadata_for_image(image_shape, pixel_sizes, dims):
    ...
    if is_3d:
        ...
        axes = create_3d_axes(
            pixel_sizes[2], pixel_sizes[1], pixel_sizes[0]
        )  # X, Y, Z order
    else:
        # Create 2D axes
    ...
    # Create multiscale
    multiscale = Multiscale(name="Generated Image", datasets=[dataset], axes=axes)
...
def example_multiscale_integration():
    ...
    # Create multiscale
    multiscale = Multiscale(name="Multiscale Pyramid", datasets=datasets, axes=axes)
