...
def example_1_compression_comparison():
    ...
    compressors = [
        ("BloscCodec + Zstd", BloscCodec(cname="zstd", clevel=5), "Recommended for most use cases"),
        ("BloscCodec + LZ4", BloscCodec(cname="lz4", clevel=3), "Fastest decompression"),
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        for name, compressor, description in compressors:
            output_path = Path(tmp_dir) / f"{name.lower().replace(' + ', '_').replace(' ', '_')}.zarr"
...
def example_2_chunking_strategies():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        for name, chunks, description in chunking_strategies:
            output_path = Path(tmp_dir) / f"chunks_{name.lower().replace(' ', '_').replace('(', '').replace(')', '')}.zarr"
...
def example_6_integration_with_zarr():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        print(f"      Resolution levels: {len([k for k in group.keys() if k.isdigit()])}")
