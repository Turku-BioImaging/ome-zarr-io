...
def test_multiscale_3d_metadata():
    ...
    first_scale = attrs_dict["ome"]["multiscales"][0]["datasets"][0]["coordinateTransformations"][0]["scale"]
    assert first_scale == [0.2, 0.1, 0.1]
    
    second_scale = attrs_dict["ome"]["multiscales"][0]["datasets"][1]["coordinateTransformations"][0]["scale"]
...
def test_validation_errors():
    ...
    with pytest.raises(ValueError, match="Translation array must have at least 2 elements"):
    ...
    axes = create_2d_axes(0.1, 0.1)
    with pytest.raises(ValueError, match="At least one scale transformation is required"):
        Dataset(
            path="0",
            coordinateTransformations=[TranslationTransformation(translation=[0.0, 0.0])]
        )
...
def test_translation_and_scale():
    ...
    coord_transforms = attrs_dict["ome"]["multiscales"][0]["datasets"][0]["coordinateTransformations"]
    assert len(coord_transforms) == 2
    
    # Find scale and translation
    scale_transform = next(t for t in coord_transforms if t["type"] == "scale")
    translation_transform = next(t for t in coord_transforms if t["type"] == "translation")
