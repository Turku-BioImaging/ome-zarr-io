...
class TestAxisUnits:
    ...
    def test_axis_units_with_dictionary(self, tmp_path):
        ...
        # Create test image
        image = np.random.randint(0, 255, size=(10, 2, 5, 64, 64), dtype=np.uint8)  # TCZYX
        dims = ["t", "c", "z", "y", "x"]
    ...
    def test_axis_units_list_length_mismatch(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="Length of axis_units list.*must match number of dimensions"):
            OmeZarrImage(
    ...
    def test_dims_length_mismatch_with_image(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="Length of dims.*must match number of image dimensions"):
            OmeZarrImage(
    ...
    def test_invalid_axis_units_type(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="axis_units must be either a list of Axis objects or a dictionary"):
            OmeZarrImage(
