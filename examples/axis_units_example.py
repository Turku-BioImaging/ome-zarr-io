...
def example_with_axis_list():
    ...
    ome_zarr_image = OmeZarrImage(
        path=output_path, image=image, dims=dims, axis_units=axis_units, overwrite=True
    )
...
def example_with_axis_dictionary():
    ...
    image = np.random.randint(
        0, 255, size=(time_points, channels, z_slices, height, width), dtype=np.uint8
    )

    print(
        f"Image shape: {image.shape} (T={time_points}, C={channels}, Z={z_slices}, Y={height}, X={width})"
    )
...
def example_with_custom_units():
    ...
    axis_units = [
        ...
        ),  # Z-axis in millimeters (larger scale)
        Axis(name="y", type="space", unit="micrometer"),  # Y-axis in micrometers
        Axis(
            name="x", type="space", unit="nanometer"
        ),  # X-axis in nanometers (smaller scale)
    ]
