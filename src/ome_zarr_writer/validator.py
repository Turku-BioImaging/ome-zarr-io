...
class OMEZarrValidator:
    ...
    def _load_schemas(self) -> None:
        ...
        schema_path = Path(__file__).parent.parent / "spec" / self.schema_version / "schemas"
        ...
        self._resolver = RefResolver(
            ...
            referrer=self._schemas.get(f"https://ngff.openmicroscopy.org/{self.schema_version}/schemas/ome_zarr.schema", {}),
            store=self._schemas
        )
    ...
    def get_validation_errors(
        ...
        for error in validator.iter_errors(metadata):
            errors.append(f"Path: {' -> '.join(str(p) for p in error.absolute_path)}, Error: {error.message}")
