...
def example_2_multichannel_3d():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        print(f"✅ Created: {output_path.name}")
        print(
            f"   Shape: {image.shape} (C={channels}, Z={z_slices}, Y={height}, X={width})"
        )
        print(
            f"   Voxel size: {scale_transformations['x']:.3f} × {scale_transformations['y']:.3f} × {scale_transformations['z']:.3f} μm"
        )
...
def example_3_time_lapse_4d():
    ...
    image = np.random.randint(
        0, 255, size=(timepoints, z_slices, height, width), dtype=np.uint8
    )
    dims = ["t", "z", "y", "x"]
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        writer.write(
            compressors=BloscCodec(
                cname="lz4", clevel=3
            ),  # Fast compression for time-lapse
            chunks=(1, 8, 128, 128),  # Chunk by single timepoint
        )

        print(f"✅ Created: {output_path.name}")
        print(
            f"   Shape: {image.shape} (T={timepoints}, Z={z_slices}, Y={height}, X={width})"
        )
        print(f"   Time interval: {scale_transformations['t']:.1f} seconds")
        print(
            f"   Total duration: {(timepoints-1) * scale_transformations['t']:.0f} seconds"
        )
...
def main():
    ...
    print(
        "Run advanced_features.py for compression, multiscale, and optimization examples."
    )
    print()
    ...
    print("Next steps:")
    print(
        "   📚 Read the documentation: https://github.com/Turku-BioImaging/ome-zarr-writer"
    )
