...
def create_multichannel_metadata_with_omero():
    ...
    scale_transform = ScaleTransformation(scale=[1.0, 0.1, 0.1])  # No scaling for channel, 0.1 μm/pixel for spatial
...
def create_with_translation():
    ...
    dataset = Dataset(
        path="0",
        coordinateTransformations=[scale_transform, translation_transform]
    )
...
def demonstrate_validation():
    ...
    try:
        ...
        dataset = Dataset(
            path="0",
            coordinateTransformations=[ScaleTransformation(scale=[0.1])]
        )
